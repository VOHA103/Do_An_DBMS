...
insert into QUANTRI values('sa','21232f297a57a5a743894a0e4a801fc3');
...
RECONFIGURE;  
GO 
 --dùng BULK INSERT để copy dữ liệu từ file text vào CSDL SQL Server
 --. Mở New Query (vì BULK INSERT là một câu lệnh T-SQL chứ không phải thực hiện trên cửa sổ Command Prompt) và đánh vào các dòng sau :

--BULK INSERT QLKhachHang.KhachHang FROM 'c:\KhachHang.txt ' WITH (DATAFILETYPE = 'CHAR')
---------------------------------------------------------------------------------Xuat tu SQL BANG pcb(xuat du lieu tu table ra txt)-----------------------------------------------
...
set @SQL = 'bcp " SELECT * FROM [QL_KHACHSAN].[dbo].DICHVU" queryout D:\DOAN_DBMS\DICHVU_SQL.txt  -w -T -t, -S ACER-THUHA -U sa -P 1003'
exec master..xp_cmdshell @SQL

SELECT*FROM DICHVU
-------------------bang khach hang 
declare @SQL1 varchar(500) 
set @SQL1 = 'bcp " SELECT * FROM [QL_KHACHSAN].[dbo].KHACHHANG" queryout D:\DOAN_DBMS\KHACHHANG_SQL.txt  -w -T -t, -S ACER-THUHA -U sa -P 1003'
...
----------bang quan tri
declare @SQ8 varchar(500)
set @SQ8= 'bcp " SELECT * FROM [QL_KHACHSAN].[dbo].QUANTRI" queryout   D:\DOAN_DBMS\QUANTRI_SQL.txt  -w -T  '
exec master..xp_cmdshell @SQ8
...
---------------------------------thu tuc xuat nhap file bang bcp
--thu tuc NHAP file khach hang
create proc SP_NhapFile_PCB @link varchar(max)
as
	begin
		declare @SQL_Nhap varchar(500) 
		set @SQL_Nhap = 'bcp  [QL_KHACHSAN].[dbo].KHACHHANG IN "'+@link+'" -T -t; -c -w'
		exec master..xp_cmdshell @SQL_Nhap
	end
go
--thu tuc xuat file khach hang
create proc SP_XuatFile_PCB @link varchar(max)
as
	begin
		declare @SQL_Nhap varchar(500) 
		set @SQL_Nhap = 'bcp  [QL_KHACHSAN].[dbo].KHACHHANG IN "'+@link+'" -T -t; -c -w'
...
go
----------------------------------------------------------thu tuc xuat hap file bang bulk
---BULK INSERT in MYSQL
CREATE PROC SP_NHAPKHACHHANG @link varchar(50)
as
	begin
		declare @SQL1 varchar(500) 
		set @SQL1 =' BULK INSERT  [QL_KHACHSAN].[dbo].KHACHHANG FROM '' '+@link+''' WITH (
		FIELDTEMINATOR='','',
		ROWTERMINATOR=''\N''
		)'
		---print @SQL1
...
--BULK INSERT in MYSQL
CREATE PROC SP_XUATKHACHHANG @link varchar(50)
as
	begin
		declare @SQL1 varchar(500) 
		set @SQL1 =' BULK INSERT  [QL_KHACHSAN].[dbo].KHACHHANG FROM '' '+@link+''' WITH (
...
SELECT*FROM KHACHHANG
GO
--Tạo proc cho tham số IN.....TIM KIEM GAN DUNG  THEO TEN KHACH HANG
create proc SP_TIMTENKHACHHANG  
AS
	select *
	from KHACHHANG
	where TenKH like '%_%'
...
GO
---–Thực thi
exec SP_TIMTENKHACHHANG 

--DROP PROC
DROP PROC SP_TIMTENKHACHHANG
GO
...
go
---2 Tạo thủ tục tim ma khach hang in ra thông tin của sinh viên có MaKH do người dùng truyền vào.
CREATE PROC SP_tim_MaKH @MaKH CHAR(20)
AS
begin
	SELECT TenKH,GioiTinh,DiaChi,SoDT
...
go
--goi thu tuc thuc thi
EXEC SP_tim_MaKH 'KH03'
--xoa thu tuc
DROP PROC SP_tim_MaKH
...
select*from NHANVIEN
go
------111:thu tuc doc danh sach cac du lieu 
create proc sp_NHANVIEN_HIENTHI
AS
SELECT*FROM NHANVIEN
GO

CREATE PROC SP_KHACHHANG_HIENTHI
AS
SELECT*FROM KHACHHANG
GO

drop proc SP_KHACHHANG_HIENTHI

CREATE PROC SP_PHONG_HIENTHI
AS
SELECT*FROM PHONG
GO
...
CREATE PROC SP_DICHVU_HIENTHI
AS
SELECT*FROM DICHVU
GO


create proc SP_QUANTRI_HIENTHI
as
	select*from QUANTRI
go
-------------------
-----------------------------------------------------------------------thu tuc them DU LIEU VAO  BANG khach hang
select*from KHACHHANG
go
...
--------------------------------------------------------------------THU TUC xoa DU LIEU VAO BANG khach hang
select*from KHACHHANG
go
create proc sp_xoa1khachhang (@makh char(10))
as
	begin
...
drop proc sp_xoa1khachhang
go
--------------------------------------------------------------------THU TUC sua DU LIEU VAO BANG khach hang
select*from KHACHHANG
go
create proc sp_sua1khachhang (
							@makh char(30),
							@tenkhach nvarchar(40),
							@gioitinh nvarchar(5),
							@diachi nvarchar(50),
...
go
drop proc sp_sua1khachhang
--------------------------------------------------------------------thu tuc them DU LIEU VAO BANG nhan vien
SELECT*FROM NHANVIEN
GO
CREATE PROC SP_THEM1NV(
							@MANV CHAR(20),
							@MATKHAU NVARCHAR(50),
							@HOTEN NVARCHAR(50),
							@GIOITINH NVARCHAR(6),
...
AS 
	BEGIN
	set dateformat dmy
	INSERT INTO NHANVIEN VALUES(@MANV,@MATKHAU,@HOTEN,@GIOITINH,@NGAYSINH,@SODT);
...
select*from NHANVIEN
--XOA THU TUC
DROP PROC SP_THEM1NV
GO	
		
--------------------------------------------------------------------THU TUC xoa DU LIEU VAO BANG nhanvien
select*from NHANVIEN
go
create proc sp_xoa1nhanvien (@manv char(10))
as
	begin
...
drop proc sp_xoa1nhanvien
go
--------------------------------------------------------------------THU TUC sua DU LIEU VAO BANG nhanvien
select*from NHANVIEN
go
create proc sp_sua1nhanvien (
							@MANV CHAR(20),
							@MATKHAU NVARCHAR(50),
							@HOTEN NVARCHAR(50),
							@GIOITINH NVARCHAR(6),
...
AS
	...
	END
GO
--------------------------------------------------------------------THU TUC xoa DU LIEU VAO BANG quan tri
select*from QUANTRI
go
create proc sp_xoa1QUANTRI (@maquantri char(10))
as
	begin
	delete from QUANTRI
	where ID=@maquantri
...
drop proc sp_xoa1QUANTRI
go
--------------------------------------------------------------------THU TUC sua DU LIEU VAO BANG quan tri
select*from QUANTRI
go
create proc sp_sua1QTri (
...
go
drop proc sp_sua1QTri		
		
		
--------------------------------------------------------------------THU TUC THEM DU LIEU VAO BANG DICH VU
SELECT*FROM DICHVU
GO
CREATE PROC SP_THEM1DV(
							@MADV CHAR(20),
							@TENDICHVU NVARCHAR(50),
							@GIADV float
							)
AS
	INSERT INTO DICHVU VALUES(@MADV,@TENDICHVU,@GIADV)
GO
drop proc SP_THEM1DV
go
--------------------------------------------------------------------THU TUC xoa DU LIEU VAO BANG DICH VU
select*from DICHVU
go
create proc sp_xoa_dv (@madv char(10))
as
	begin
	delete from DICHVU
	where MaDV=@madv
...
drop proc sp_xoa_dv
go
--------------------------------------------------------------------THU TUC sua DU LIEU VAO BANG DICH VU
select*from DICHVU
go
create proc sp_update_dv (
							@MADV CHAR(20),
							@TENDICHVU NVARCHAR(50),
							@GIADV float
							)
...
go
drop proc sp_update_dv
go


create proc sp_update_dv1 (
...
go
drop proc sp_update_dv1
go
------------------------------------------------------------------------------------------CÁC HÀM (FUNCTION)--------------------------------------------------------------------
-----:dem so luong khach hang tren table
select*from KHACHHANG
go
create function FC_COUNT_KH()
returns int
as
	begin
	declare @khh int
	set @khh=(select COUNT(*) from KHACHHANG)
	return @khh
	end
go

---xoa function
drop function FC_COUNT_KH
-----thuc thi
...
---11.Viết trigger khi xoá 1 dòng dữ liệu trong bảng dich vu thì trong bảng sddich vu do cũng xoá theo.

select*from DICHVU
select*from SDDICHVU
go
CREATE TRIGGER TRG_XOA_DV ON DICHVU
INSTEAD OF DELETE
AS
BEGIN
...
END
---thuc thi
DELETE DICHVU WHERE MaDV='DV02'
----XOA TRIGGER
DROP TRIGGER TRG_XOA_DV
GO
...
go
----------------------------------------------------------CÁC CURSOR---------------------------------------------------------------------------
------xem danh sach quan tri 
create proc CS_QUANTRI1
as
begin
	DECLARE CS_QUANTRI CURSOR
	global
	dynamic
FOR 
	SELECT ID,TENTK,MATKHAU  from QUANTRI
	
	open CS_QUANTRI

	FETCH NEXT FROM CS_QUANTRI
	...
	close CS_QUANTRI
	deallocate CS_QUANTRI
end 
go

drop proc CS_QUANTRI1
...
RETURNS @BANGTHONGKE TABLE(TENKHACH NVARCHAR(50),DIACHI NVARCHAR(50),SODT CHAR(15),MAPHONG CHAR(10),NGAYVAO DATE ,NGAYRA DATE,TIENTHUE INT)
AS
	BEGIN
		DECLARE CS_CAU2 CURSOR 
		FOR SELECT KH.TenKH,KH.DiaChi,KH.SoDT,TH.MaPhong,TH.NgayVao,TH.NgayRa,TH.tienThue FROM KHACHHANG KH,THUEPHONG TH WHERE KH.MaKH=TH.MaKH AND KH.MaKH=@MAKH
		OPEN CS_CAU2
		DECLARE @TENKHACH NVARCHAR(50),@DIACHI NVARCHAR(50),@SODT CHAR(15),@MAPHONG CHAR(10),@NGAYVAO DATE ,@NGAYRA DATE,@tienthue int
		FETCH NEXT FROM CS_CAU2 INTO @TENKHACH,@DIACHI,@SODT,@MAPHONG,@NGAYVAO,@NGAYRA,@tienthue
		WHILE(@@FETCH_STATUS=0)
		BEGIN

		INSERT INTO @BANGTHONGKE SELECT @TENKHACH,@DIACHI,@SODT,@MAPHONG,@NGAYVAO,@NGAYRA,@tienthue
		FETCH NEXT FROM CS_CAU2 INTO @TENKHACH,@DIACHI,@SODT,@MAPHONG,@NGAYVAO,@NGAYRA,@tienthue
		END
	CLOSE CS_CAU2
	DEALLOCATE CS_CAU2
